<div class="w-full relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    ...
    @if (strlen($search) > 2)
        <div class="absolute bg-gray-800 text-sm rounded w-full lg:w-64 mt-4" x-show="isOpen" x-cloak>
                        ...
                        <li class="border-b border-gray-700  px-4">
                            @foreach ($searchResults as $result)
                                <a @if ($loop->last) @keydown.tab="isOpen = false" @endif
                                    ...
                                    class="block hover:bg-gray-700 py-3 flex">
                                    @if ($result['poster_path'])
                                        <img src="https://image.tmdb.org/t/p/w92/{{ $result['poster_path'] }}"
                                            class="w-8" alt="">
                                        <span class="text-lg px-2">{{ $result['title'] }}</span>
                                </a>
                            @else
                        ...
                        @foreach ($searchTvResults as $result)
                            <a @if ($loop->last) @keydown.tab="isOpen = false" @endif
                                ...
                                @if ($result['poster_path'])
                                    <img src="https://image.tmdb.org/t/p/w92/{{ $result['poster_path'] }}"
                                        class="w-8" alt="">
                                    <span class="text-lg px-2">{{ $result['name'] }}</span>
                            </a>
                        @else
                            <img src="{{ asset('img/blank_movie_poster.jfif') }}" class="w-8" alt="">
